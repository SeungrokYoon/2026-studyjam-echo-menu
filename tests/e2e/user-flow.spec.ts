...
async function doubleTapTouchpad(page: Page) {
...
test("모바일 주문 흐름을 명시적 버튼만으로 완료한다", async ({ page }) => {
...
test("기여 기능은 주문 화면과 분리되어 동작한다", async ({ page }) => {
  ...
  await page.locator("#nav-contribute").click();
  await expect(page.locator("#contribute-view")).toBeVisible();
  await expect(page.locator("#assist-view")).toBeHidden();

  await page.locator("#contrib-username").fill("접근성테스터");
  await page.locator("#contrib-venue").fill("스타벅스 강남역점");
  await page.locator("#contrib-search-results .search-result-option").first().click();
